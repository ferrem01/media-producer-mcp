...
- Deeper design docs: `SPEC-motion-architecture.md` (motion layers, one-camera
  ...
  `ROADMAP.md`, `SPEC-studio.md`, `SPEC-brand-extraction.md`, `SPEC-team.md` (who shares a tenant: invite, membership, company domain), `SPEC-briefs.md` (a marketing brief in, a film out: what a brief locks, real footage on any grammar), `SPEC-recipes.md` (the third axis: a measured cut the writer fills -- grammar, frame, recipe), `SPEC-metamorph.md` (DESIGN: a oner carried by objects -- handoff lane, `data-morph` tags, rack focus; components get tags, not morph states).
