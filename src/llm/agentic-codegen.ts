...
7. **THE TYPE SYSTEM — TWO SANCTIONED ENTRANCES, NOTHING ELSE.** Editorial type enters ONE of two ways. (a) LINE-RISE (default): the whole line rises ~10px and fades in over ~300ms (\`mpLineRise\`), stagger per LINE. (b) TYPE-ON (tempo-cut build-up): words APPEAR in place, in reading order, on a fixed beat interval — no fade, no travel, the centered line reflows as it grows (kinetic-text \`entrance:'type-on'\`; the ONLY variable is time). Everything else stays banned in both modes: no masks/clip reveals, no rotation, no scale pops, no glow, no direction variety between scenes, no per-word styling. Typing (char-by-char with a cursor) is allowed ONLY inside UI surfaces — composers, prompts, terminals — where typing is what the surface does. Emphasis is COLOR ONLY (one brand accent); an accented word enters exactly like every other word. Numbers are the only type allowed to perform (count-up via \`mpCountUp\`). Exits are fades or cuts — nothing floats away. Per-word choreography (motion, masks, mixed directions) is the #1 amateur tell and is a failure.

8. **TYPE ON PHOTOGRAPHS, NEVER CARDS.** When the scene has a photographic world (hero image, backdrop image, b-roll), do NOT stack opaque content cards/panels over the photo — that is instant PowerPoint. Add a diffuse scrim (a soft gradient wash or gentle blur-darkening over the image region behind the text; \`mpAtmosphere\`'s image mode provides one) and set the typography DIRECTLY on the image: large white or deep-ink type with generous tracking, small uppercase kickers, thin rules. Text lives IN the photo's world; boxes float ON TOP of it and are a failure. (Rule 1's contrast bar still applies — the scrim is what earns it.)
...
- \`mpWordReveal\` -- DEPRECATED for editorial type (masked per-word reveal violates the type system). For a word-by-word build-up use the kinetic-text component with \`entrance:'type-on'\` instead.
