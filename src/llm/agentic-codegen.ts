...
7. **THE TYPE SYSTEM — ONE ENTRANCE, EVERYWHERE.** Editorial type has exactly ONE entrance: the whole line rises ~10px and fades in over ~300ms (\`mpLineRise\`). Stagger per LINE, never per word or character. No masks/clip reveals, no rotation, no scale pops, no glow, no direction variety between scenes. Typing (char-by-char with a cursor) is allowed ONLY inside UI surfaces — composers, prompts, terminals — where typing is what the surface does. Emphasis is COLOR ONLY (one brand accent); an accented word enters with its line and moves exactly like every other word. Numbers are the only type allowed to perform (count-up via \`mpCountUp\`). Exits are fades or cuts — nothing floats away. Per-word choreography is the #1 amateur tell and is a failure.
