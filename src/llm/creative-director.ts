...
export interface VisualSystem {
  ...
  world?: "light" | "dark" | "paper" | "plain" | "sky" | "cream";
...
- "creator-cut": a person explains and the SCREEN PROVES IT (SPEC-creator-cut.md; the creator tutorial / creator ad shape). The person is the spine -- one continuous take or one take per scene, the voice is the clock, voiceover_text carries the exact spoken lines -- but the edit is BUSY: every scene is one CLAIM, and each beat names the EVIDENCE the screen shows while the claim is said (a screenshot, a screen recording, b-roll, a product mock). The proof comes in as a CUTAWAY -- it takes the frame for the beat, hard cut in and out -- and the film cuts back to the person between every piece of proof; a chapter label is a graphic on its claim, gone when the claim moves on; the camera punches in on the person on the claim and pulls back on the turn; an icon sticker beside the head names the thing being talked about. No standing title, no CTA or sponsor line unless the brief asks. Ad vs tutorial is the same grammar at a different motion and length: punchy and ~30s for an ad, calm and 60-90s for a tutorial. Sound is the voice: audioSystem.music_mood "none" for a tutorial, a bed ducked far under an ad. Choose it when a person is explaining a product and the argument needs the screen to answer every sentence; choose "speaker" when the person alone carries it with one clean graphic per beat.
...
- VISUAL SYSTEM (committed): ${[
    ...
    bible.visualSystem.motion ? `motion=${bible.visualSystem.motion}${bible.visualSystem.motion === "cutout-physics" ? " (elements move as rigid flat pieces: drop, settle, swing -- no glows, no morphs, no elastic scaling). The 12fps stop-motion stepping and the ink boil are applied MECHANICALLY to the whole scene at assembly time -- do NOT hand-roll stepped easing, frame-holds or jitter, you would only step it twice" : bible.visualSystem.motion === "calm" ? " (settle, never bounce; entrances ease out and land; nothing overshoots or tilts)" : ""}` : null,
