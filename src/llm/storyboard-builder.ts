...
import { recipeBlock, recipeSceneBand } from "../core/recipes.js";
...
const SCENE_TOOL_SCHEMA = {
  ...
  properties: {
    ...
    assets: {
      type: "array",
      description: "ANY FILM for a LIVE-ACTION CAMEO (a person on camera for ONE beat of a film no person carries -- a founder's two-second line on a sketch): {type: \"camera_video\", use: \"clip\", description: who and what they do, recording_instructions: the line} -- the team records it in the booth and the build places the clip on that scene as a video; never stock_footage of 'a founder'. ANY FILM for stock_footage: when the visual direction asks for real footage (an office, a team at work, a place, a face in a crowd) add {type: \"stock_footage\", description: the shot as a search phrase} to that scene -- the build fetches the clip and lays it as the scene's ground under the type and the cards; a gradient where footage was asked for is a dropped direction. ANY FILM for screen_recording / screenshot: wherever a scene stages a product mock as its payoff (quotient-*, slack-workspace, any real-product surface), also list {type: \"screen_recording\", description: the screen and what happens on it} -- the build casts a SLATE in the mock's slot (a plain card naming the recording it waits for; the mock never ships pretending to be the real screen), and the team's real recording takes that slot when it is uploaded. Still cast the mock: it tells the build where the screen goes and how it is timed. CREATOR-CUT FILMS for the rest: the proof the screen shows while this claim is said, one entry per beat: {type, description, use?, at?, until?, focus?}. type: screenshot | screen_recording (the human supplies these through the board) | stock_footage | mockup | illustration (the build makes these -- illustration: a drawn object for a claim no screen can prove). description: what it shows, specific enough to go and find it -- for an illustration, the one object in one sentence. use: cutaway (default -- takes the frame, hard cut in/out) | split (on a tall frame the screen owns the top of the frame and the person stays under it) | card (floats over the person; only when the brief asks). at/until: word anchors into voiceover_text (\"@dashboard\"). focus: where the eye should go on it, in words.",
      items: {
        type: "object",
        properties: {
...
export function normalizeSceneShape(scene: any, validTypes?: Set<string>): string[] {
  ...
  // Proof the writer asked for (creator-cut) lands on the scene's needs:
  // known types with a real description survive as full need records
  // (core/asset-needs.ts); a hydrated board's needs pass through intact.
  if (scene.assets !== undefined) {
    const needs = normalizeAssetNeeds(scene.assets);
    if (Array.isArray(scene.assets) && needs.length < scene.assets.length) notes.push(`dropped ${scene.assets.length - needs.length} asset need(s) with no known type or no description`);
    if (needs.length) scene.assets = needs; else delete scene.assets;
  }
...
export interface StoryboardBuilderOpts {
  ...
  /** THE RECIPE (SPEC-recipes.md): the measured cut this board must fill.
   *  Its spine sets the scene budget and each beat's seconds and words. */
  recipe?: import("../core/recipes.js").Recipe;
}
...
export interface DraftScene {
  ...
  beats?: SceneBeat[];
  /** The scene's needs: the take, and on a creator-cut board the proof each
   *  claim wants on screen with its words (SPEC-creator-cut.md). */
 ...
 *  whose own contract says "text is the voiceover, no narrator" (hype-cut,
 *  tempo-cut). The recipe is the measured cut of THAT film; its voice is
 *  part of the cut. The pipeline's voiceover strip honors the same rule.
 *  Measured live (proj_ff9e68e4): the launch recipe on hype-cut came back
 *  silent, every beat's line living only in type. */
export function recipeWantsVoice(recipe: { layers?: Record<string, unknown> } | undefined | null): boolean {
  return String(recipe?.layers?.voice || "") === "voiceover";
}
...
export async function buildStoryboard(opts: StoryboardBuilderOpts): Promise<StoryboardResult> {
  ...
  // THE RECIPE SETS THE BUDGET when there is one: its beats, repeats included.
  if (opts.recipe) band = recipeSceneBand(opts.recipe);
...
- TEXT IS THE VOICEOVER: no narrator. Every narration line lives in an annotation or kinetic-text STAMPED OVER the surface's lower third -- never a statement slide mid-film (title open and CTA close are the only pure-type scenes). Long lines use kinetic-text entrance:"type-on" with word_interval on the beat subdivision.
- THE FRAME BELONGS TO ONE THING: a product beat's surface fills the frame (~84% inset); claim/annotation type stamps OVER its lower third or waits for its own cut. NEVER a side-by-side layout -- window docked one side, type column the other -- that is a slide, not a film (the layout engine refuses it in this grammar; author accordingly).
...
- ONE GAG maximum: a single struck-through gag card ("we killed ___") beside a setup line, if the story earns a joke. One, or none -- a second reads as a tic.
- THE OBJECT BEHIND THE WORDS: a pure-type beat that names a thing the viewer can picture (the hours, the pile-up, the weekend back) carries it as an illustrated object: set the scene's "hero_image" to the one object in one sentence, in the film's palette, and the line rides over it on a plate (the world's display type, one *starred* word, "ring": true when the story points at it). A dark field with words alone is the slide this grammar exists to kill.
...
- BUILD FROM THE KIT, NEVER FROM SCRATCH: every tempo-cut scene is composed from library components or an st-template, cast from THE WORLD's materials named above. Do NOT invent custom scenes in this grammar: hand-rolled surfaces are how corner labels, theme whiplash and clipped type sneak back in. NO montage/scene-name labels anywhere.` : ""}
...
- THE ALTERNATION AT PACE: kinetic type interstitials alternate with product beats. An interstitial is an st-statement scene (ONE thought, one *starred* emphasis, ~1 bar / 1-1.5s -- read once, cut); statement slides are not merely allowed mid-film here -- they ARE the connective tissue (this overrides tempo-cut's no-statement-slides law). A product beat is a named product mock performing (2-6 bars) that OWNS THE WHOLE FRAME. The interstitials carry the claims, so a product beat rarely needs claim type at all -- when it does, the type stamps over the surface's lower third, NEVER a side column beside a docked window (that split-frame is the layout this grammar exists to kill).
...
- THE OBJECT BEHIND THE WORDS (story ads, Gamma-style): a type beat that names a thing the viewer can picture -- the hours, the mess, the weekend -- carries that thing as an illustrated object behind the line: set the scene's "hero_image" to the one object in one sentence, in the film's palette ("a pink alarm clock, flat illustrated, on a deep blue ground"), and cast the line as the world's display type with one *starred* word and "ring": true when the story points at it (a hand-drawn loop circles the line once it lands). The object is the beat's world; the words ride over it on a plate. Two or three such beats per film, never every beat.
- CASTING: interstitials are st-statement ONLY (editorial's component, played at hype pace) -- or, on an object beat, the display type over the hero_image. Product beats stage the SPECIFIC named mock mid-performance with real product chrome (real connector logos on tool chips); a generic simulator standing in for a named product is a blocking generic_surface defect, same as tempo-cut.
` : ""}
...
- THE DATA KIT: number-counter-row (counting rows), stat-card (one hero figure + label), bar-chart, line-chart, progress-bar, funnel-chart (staged conversion -- sent/delivered/opened/clicked -- bars sweep in sequence with count-ups and step-conversion badges; the LAST stage is styled as the money number), metric-dashboard (finale only), flowchart (mechanism beats). Charts carry data.values with the REAL figures; visual_notes name the draw choreography (which bar lands first, where the line pauses before the spike).
...
- THE CONTENT TRAVELS, AND THAT IS THE EDIT: there are no hard cuts, and the illusion is built by SUBJECTS moving through the frame -- not by the camera. The stage camera is rebuilt for every scene, so it cannot carry anything across a boundary; only content can. AUTHOR THE DIRECTION AS DATA on each component, never only as prose: the outgoing element gets an "exit" and the incoming element of the NEXT scene gets the matching "enter" from the opposite edge, so the two beats read as one continuous move. Something that exits "slide-left" is arriving from the right, so the next scene's subject enters "slide-right"; up pairs with down. Example -- scene N: {"type":"<the thing leaving>","data":{...},"exit":"slide-left"}, then scene N+1: {"type":"<the thing arriving>","data":{...},"enter":"slide-right"}. (Cast the actual types from THE WORLD's materials -- the DIRECTION is this grammar's business, the PARTS are the world's.) Vocabulary: slide-left, slide-right, slide-up, slide-down (the travelling ones), plus rise, pop, fade. Say the same move in visual_notes as well, for the codegen. The KINETIC CONTINUITY rules above are not optional here; they are the grammar.
- THE WHOLE PLACE CROSSES, AND IT NEVER PARKS: the crossing belongs to the BEAT, not to one element. When a camera passes a page, everything ON the page travels together at the same rate -- so EVERY component in a scene except the surface carries the SAME "enter" and the SAME opposite "exit". One thing moving while the rest hold still reads as that THING moving; everything moving together reads as the VIEWER moving, and that difference is the entire grammar. The pair is ONE crossing and is played as one: a quick approach, a long slow pass through the middle where the beat is read, a quick departure -- so a subject that arrives, stops, waits out the scene and then leaves is not this grammar at all. Enters "slide-left" pairs with exits "slide-right"; up pairs with down. Hold ONE direction for the whole film (all left-to-right, or all right-to-left) so every scene picks up exactly where the last one left off. MANDATORY, every scene: each non-surface component carries both ends. The only exemptions are the first scene, which may omit the "enter" because nothing precedes it, and the last, which may omit the "exit" because it is the film's one true stop. A beat that holds still is the slideshow tell.
...
- ONE ELEMENT CARRIES THE FILM: pick a single physical device (a drawn line, a prop, a sheet, a cursor) that persists across every place and stitches them together. Name it in scene 1 and keep it in frame. This law is about RECURRENCE, not motion: the carrier does NOT own the direction, and giving it the crossing while the rest of the scene holds still is the failure this grammar is most prone to. It crosses with everything else on the surface.
...
- THE IDEA BEAT, WHEN NO SCREEN CAN PROVE IT: a claim about money, time, a person, a place or a feeling has no product surface. Then the proof is a DRAWN OBJECT: add a need in "assets" -- {type: "illustration", description: the one object in one sentence (a stack of cash on a plain ground; a house at dusk; a chess pawn), at: "@word", until: "@word", focus: where the eye goes} -- and the build draws it in-house and cuts it in on those words like any mock. Over it, cast the figure as a plated label in the world's display type, cut in on the same words ("$1.4 million", "3 hours"), with "ring": true so a hand-drawn loop circles it once it lands. Never a screen for a claim a screen cannot show; never an illustration for a claim the product performs.
- THE WORLD BEAT, WHEN THE LINE IS ABOUT PEOPLE OR A PLACE: "the team", "the office", "a customer call", "the whole company" -- a line that names real people doing real things, with no product surface and no single object to draw. Then the proof is FOUND FOOTAGE: add a need in "assets" -- {type: "stock_footage", description: the shot as a search phrase (a small startup team talking around a table in a bright office; two coworkers laughing at a standing desk), at: "@firstword", until: "@lastword"} -- and the build finds it (portrait on a tall frame) and cuts it in on those words while the captions keep running. The person speaks over it; no sticker, no label. A founder-story film has one or two of these; a product beat never does.
- THE SPLIT, WHEN THE PERSON STAYS ON WHILE THE SCREEN PROVES IT: on a tall frame a long product beat (more than ~5s of the screen performing) or a beat where the person's face should keep selling the line keeps the person on screen: the screen owns the top of the frame, flush, and the person the bottom, captions on the person. Ask for it with "use": "split" on the proof need (a screenshot or recording), or "use": "split" in a library mock's data when the mock is the proof. Short claims stay cutaways (the hard cut is the rhythm); the split is for the beat that lingers on the screen.
...
- Metrics / numbers / results -> stat-card, number-counter-row, metric-dashboard, bar-chart, line-chart, progress-bar, funnel-chart (multi-stage conversion story in one draw). A HERO stat gains physicality from floating-pills (data chips drifting around it in faux-3D).
...
export function enforceFilmDirection(scenes: DraftScene[]): void {
  ...
  const BACKDROPS = new Set(["paper-ground", "sky-backdrop", "cream-ground", "webgl-backdrop", "mesh-gradient", "gradient-background", "liquid-background", "animated-gradient-text"]);
  for (const s of scenes) {
