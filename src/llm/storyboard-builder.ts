...
const SCENE_TOOL_SCHEMA = {
  ...
  properties: {
    ...
    assets: {
      type: "array",
      description: "CREATOR-CUT FILMS ONLY. The proof the screen shows while this claim is said, one entry per beat: {type, description, use?, at?, until?, focus?}. type: screenshot | screen_recording (the human supplies these through the board) | stock_footage | mockup | illustration (the build makes these -- illustration: a drawn object for a claim no screen can prove). description: what it shows, specific enough to go and find it -- for an illustration, the one object in one sentence. use: cutaway (default -- takes the frame, hard cut in/out) | split (on a tall frame the screen owns the top of the frame and the person stays under it) | card (floats over the person; only when the brief asks). at/until: word anchors into voiceover_text (\"@dashboard\"). focus: where the eye should go on it, in words.",
      items: {
        type: "object",
        properties: {
