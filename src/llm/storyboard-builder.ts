...
- THE SUBJECT CROSSES, IT NEVER PARKS: give the travelling subject BOTH an "enter" and the OPPOSITE "exit" -- enters "slide-left" and exits "slide-right", or enters "slide-up" and exits "slide-down". That pair is ONE crossing, and it is played as one: a quick approach, a long slow pass through the middle where the beat is read, a quick departure. A subject that arrives, stops, waits out the scene and then leaves is an OBJECT that slid in; a subject that never stops moving is a CAMERA passing it, and that difference is the whole grammar. So every travelling component carries both ends, and the direction stays the same for the whole film (all left-to-right, or all right-to-left) so each scene picks up exactly where the last one left off.
