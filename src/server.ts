...
export function createMcpServer(): McpServer {
  ...
  tool(
    ...
    "Create a new media project (video, image, presentation, one-pager, slideshow, gif, social)",
    {
      ...
      recipe: z.string().optional().describe("The RECIPE axis (SPEC-recipes.md): the measured cut the writer fills -- an id from the library (presenter-n-things, presenter-split-tour, founder-story-broll, speaker-kinetic-claims, speaker-one-take-cards). A recipe belongs to one grammar and implies it. Omit to let the director pick one that suits the brief, or none."),
